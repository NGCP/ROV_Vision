...
static void fillAddr(const string &address, unsigned short port, sockaddr_in &addr) {
  ...
  try {
cerr << endl << "&addr: " << &addr << endl;
cerr << endl << "&address: " << &address << endl;
...
//cerr << "Failed to resolve name gethostbyname" << endl;
      throw SocketException("Failed to resolve name (gethostbyname())");
    }
//cerr << "*((unsigned long *) host->h_addr_list[0]): " << *((unsigned long *) host->h_addr_list[0]) << endl;

//if (*((unsigned long *) host->h_addr_list[0]) == NULL) {
//  cerr << "NULL THINGY" << endl << endl;
//}
    addr.sin_addr.s_addr = *((unsigned long *) host->h_addr_list[0]);
//cerr << "addr.sin_addr.s_addr " << addr.sin_addr.s_addr << endl;
//if (port == NULL) {
//  cerr << "NULL htons(port)" << endl << endl;
//}
//cerr << "htons(port): " << htons(port) << endl;

    addr.sin_port = htons(port);     // Assign port in network byte order
//if (addr.sin_port == NULL) {
//  cerr << "NULL addr.sin_port" << endl << endl;
//}
//cerr << "addr.sin_port " << addr.sin_port << endl;
  }
