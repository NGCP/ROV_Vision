...
void sendStream(string servAddress, unsigned short servPort, int camNum) {
    try {
       UDPSocket sock;
       ...
       //open video0 or video1 depending on value of cam
       run(camNum, &fd);
       ...
       for(;;) {
          ...
          struct timeval tv = {0};
          tv.tv_sec = 10;
          std::cerr << "Before Sync " << camNum << std::endl;
          syncher.Sync();
          std::cerr << "After Sync " << camNum << std::endl;
          int r = select(fd+1, &fds, NULL, NULL, &tv);
          std::cerr << r << std::endl;
          if(-1 == r)
          {
